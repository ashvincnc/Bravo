 ...
 #            control = 1
    ...
    def pwm_in(self):
           ...
           #     print('start',self.start_time)
                ...
                self.pPWM.ChangeDutyCycle(40)#self.pressureCycleValue[0])
                self.o2PWM.ChangeDutyCycle(40)#self.pressureCycleValue[1])
...
class backendWorker(QThread):
    ...
    def getdata(self):
        ...
        global emergency,p_s,flag,p_value
        ...
        try:    
#            for i in range(4):
            data[0] = adc.read_adc(0, gain=GAIN)
        except:
            print('| {0:>6} | {1:>6} | {2:>6} | {3:>6} |'.format(*data))
        p_data = (data[0]*4.096)/32767    
        if(mod_val in [2]):    
            
            p_data = round(p_data,1)
            print('sensor_data',p_data)
            print('len',len(p_s))
            print('p_s',p_s)
            if (len(p_s)) < 3:    
                p_s.append(p_data)
